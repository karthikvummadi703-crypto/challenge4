...
/**
 * Lazy-loaded application shell — owns ALL Firebase-dependent providers.
 *
 * This module is loaded as a separate async chunk so that `firebase/auth`
 * and `firebase/firestore` are NOT included in the critical-path entry
 * bundle.  React mounts and shows the `AppLoadingFallback` spinner in
 * App.tsx while this chunk (plus Firebase) downloads in parallel.  By the
 * time the SplashScreen animation completes (~2 s), the chunk is already
 * in the browser cache.
 *
 * Import chain:
 *   AuthenticatedApp → AuthProvider → authContext → firebase/auth, ../firebase
 *   AuthenticatedApp → DemoModeProvider → demoModeContext → authContext (same)
 */
...
/**
 * Inner application content — manages role routing, splash state, and the
 * shared background layers.  Must live inside AuthProvider + DemoModeProvider.
 */
function AppContent() {
...
/**
 * Root provider wrapper — exported as the lazy-loaded default so App.tsx
 * can split Firebase out of the critical-path bundle.
 */
